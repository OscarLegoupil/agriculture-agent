...
## Broader known-data check: reject land-only promotion

The frozen land-only artifact completed all 128 authorized games on seeds 3000-3031. Its narrow-screen improvement does not generalize.

| Opponent | v8 score | Land-only score | Mean gap: v8 / land-only | Median gap: v8 / land-only | 10th percentile gap: v8 / land-only |
|---|---:|---:|---:|---:|---:|
| COK | 21/64 | 19/64 | -4,732 / -6,921 | -5,981 / -10,213 | -12,328 / -16,314 |
| Seyam | 59/64 | 60/64 | +16,609 / +18,275 | +15,973 / +16,883 | +3,250 / +3,972 |

With equal opponent weights, match score changes from 62.50% to 61.72%. The paired change is -0.78 percentage points, with a 95% seed-cluster bootstrap interval of [-8.59, +7.03] points. Both seats and opponents remain in each sampled seed cluster. This uncertainty interval describes this fixed opponent pool, not uncertainty about the live field. The COK family remains below the declared 40% floor, so the artifact is not promoted.

All 128 games buy three additional plots and reach a daily sampled peak of 68 productive cells. Mean successful travel falls from 4,165.64 to 4,032.06 against COK and from 4,203.83 to 4,034.47 against Seyam. This measurable travel reduction is insufficient to offset capital allocation and production-market effects. Mean day-15 crops remain exactly seven wheat and 43 strawberries against both opponents. The extra plot changes placement, not feed-production capacity; the separate larger-crop screen mostly expands recurring crops and cannot establish the value of a larger commercial/feed wheat cohort.

Mean labor expense is 7,926 against COK and 8,220 against Seyam; wheat expense is 10,058 and 10,735. There are 21 nonterminal escapes, 260 terminal escapes, 22 water deaths and zero overflow across the full panel. All games complete, with zero candidate/opponent errors, failed work or candidate stderr. Maximum measured action time is 246.8 ms. Terminal escapes remain separately labeled rather than automatically counted as scheduler failures.

The evidence supports investigating what the fourth plot produces and how its inputs are serviced, not asserting that additional area itself improves competitive outcomes. Further land-only ablations are not justified by this completed test.

Reproduction and exact paired provenance are in [phase4-land-field-paired.json](results/phase4-land-field-paired.json) and [the realized behavior summary](results/phase4-land-field-summary.json). The complete manifest is `data/raw/phase4-land-field.json`; no outcomes or candidate bytes were altered when adding its explicit single-candidate identity fields for the shared comparison tool.

```powershell
python scripts/phase4_land.py --summarize --output data/raw/phase4-land-field.json
python scripts/phase4_compare.py reports/results/phase3-validation-challenger.json.gz data/raw/phase4-land-field.json --seeds (3000..3031) --opponents data/raw/reference-cok/main.py data/raw/reference-seyam/main.py --output reports/results/phase4-land-field-paired.json
```
