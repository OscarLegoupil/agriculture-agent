"""Baseline v4: v3 plus a second carrot tile and one hired hand.

Adds a second CARROT tile on (3, 3) inside the starting NW quadrant and hires
one farm hand per day (first Fibonacci step: $1). The hand tends the new tile
primarily so the farmer can keep running the coop and the original crops.

Land expansion is intentionally not triggered. In a 30-day two-player game at
this production scale, buying NE at $1000 does not pay for itself: v3 already
runs profitably inside NW and the hand covers the extra tile without extra
land. Later baselines can revisit land purchases once scale justifies them.
"""
...
_CARROT_B = (3, 3)  # CARROT #2 (already unlocked NW quadrant)
...
def _hand_op(
    ...
    """Hand handles CARROT_B primarily; falls back to any watering elsewhere."""
...
def agent(obs: dict[str, Any]) -> dict[str, Any]:
    ...
    # HIRE one hand per day starting day 3 (first Fibonacci step is $1).
